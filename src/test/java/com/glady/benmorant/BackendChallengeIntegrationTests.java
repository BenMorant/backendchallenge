...
class BackendChallengeIntegrationTests {
    ...
    void should_give_the_correct_output_for_gift_and_the_correct_user_balance_when_now_is_before_the_expiration_date() {
        ...
        long userBalance = userBalanceService.calculateUserBalance(item);

        String actualOutput = user.getName() + " receives a " + item.getItemType().getName() + " distribution with the amount of $" + item.getAmount() + " from " + company.getName() + ". He will therefore have $" + userBalance + " in gift cards in his account. He received it on " + dtf.format(item.getDistributionDate()) + ". The gift distribution will expire on " + dtf.format(item.getExpirationDate()) + ".";
        // THEN
